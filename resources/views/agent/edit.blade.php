        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-10 text-gray-900">
                    ...
                    <form action="{{ route('vehicles.update', ['vehicle' => $vehicle->id]) }}" method="post" class="max-w-lg">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                                ...
                                <select name="companyId" class="block mt-2 w-full border-gray-300 focus:ring-0 focus:border-gray-500">
                                    ...
                                    <option value="{{ $company->id }}" selected="{{ $company->id == $vehicle->companyId?'true':''}}">{{ $company->companyname }}</option>
                                    @endforeach
                                </select>
                            </div>
                        <div>
                            <x-input-label for="vehiclereg" :value="__('vehiclereg')" />
                            <x-text-input value="{{$vehicle->vehiclereg}}" id="vehiclereg" name="vehiclereg" type="text" class="mt-1 block w-full"  required autofocus autocomplete="vehiclereg" />
                            <x-input-error class="mt-2" :messages="$errors->get('vehiclereg')" />
                        </div>
                        <div>
                            <x-input-label for="vehiclechasis" :value="__('vehiclechasis')" />
                            <x-text-input value="{{$vehicle->vehiclechasis}}" id="vehiclachasis" name="vehiclechasis" type="text" class="mt-1 block w-full"  required autofocus autocomplete="vehiclechasis" />
                            <x-input-error class="mt-2" :messages="$errors->get('vehiclechasis')" />
                        </div>
                         
                        <div>
                            <x-input-label for="gpsId" :value="__('GPSId')" />
                            <x-text-input value="{{$vehicle->gpsId}}" id="gpsId" name="gpsId" type="text" class="mt-1 block w-full"  required autofocus autocomplete="gpsId" />
